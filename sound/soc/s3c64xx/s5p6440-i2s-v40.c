...
static struct s3c24xx_pcm_dma_params s5p64xx_i2s_pcm_stereo_out = {
	.client		= &s3c24xx_dma_client_out,
	.channel	= DMACH_I2S_V40_OUT,
	.dma_addr	= S5P64XX_PA_IIS_V40 + S3C64XX_IISFIFO,
	.dma_size	= 4,
};

static struct s3c24xx_pcm_dma_params s5p64xx_i2s_pcm_stereo_in = {
	.client		= &s3c24xx_dma_client_in,
	.channel	= DMACH_I2S_V40_IN,
	.dma_addr	= S5P64XX_PA_IIS_V40 + S3C64XX_IISFIFORX,
	.dma_size	= 4,
};

struct s5p64xx_i2s_info {
	void __iomem	*regs;
	struct clk	*iis_clk;
	int master;
};
static struct s5p64xx_i2s_info s5p64xx_i2s;
...
static void s3c24xx_snd_txctrl(int on)
{
	...
	iiscon  = readl(s5p64xx_i2s.regs + S3C64XX_IIS0CON);
	iismod  = readl(s5p64xx_i2s.regs + S3C64XX_IIS0MOD);
	...
	if (on) {
		...
		writel(iismod,  s5p64xx_i2s.regs + S3C64XX_IIS0MOD);
		writel(iiscon,  s5p64xx_i2s.regs + S3C64XX_IIS0CON);
...
static void s3c24xx_snd_rxctrl(int on)
{
	...
	iisfcon = readl(s5p64xx_i2s.regs + S3C64XX_IIS0FIC);
	iiscon  = readl(s5p64xx_i2s.regs + S3C64XX_IIS0CON);
	iismod  = readl(s5p64xx_i2s.regs + S3C64XX_IIS0MOD);
	...
	if (on) {
		...
		writel(iismod,  s5p64xx_i2s.regs + S3C64XX_IIS0MOD);
		writel(iisfcon, s5p64xx_i2s.regs + S3C64XX_IIS0FIC);
		writel(iiscon,  s5p64xx_i2s.regs + S3C64XX_IIS0CON);
	} else {
		/* note, we have to disable the FIFOs otherwise bad things
		 * seem to happen when the DMA stops. According to the
...
static inline int s3c24xx_snd_is_clkmaster(void)
{
	...
	return (readl(s5p64xx_i2s.regs + S3C64XX_IIS0MOD) & S3C64XX_IISMOD_SLAVE) ? 0:1;
}
				...
				struct snd_pcm_hw_params *params)
{
	...
	case 2:
		...
		break;
	default:
		printk(KERN_ERR "s5p-i2s-v40: %d channels unsupported\n",
	...
	s3cdbg("s3c iis mode: 0x%08x\n", readl(s5p64xx_i2s.regs + S3C64XX_IIS0MOD));
...
static void s5p64xx_i2s_shutdown(struct snd_pcm_substream *substream)
{
	 ...
	 */
	__raw_writel(__raw_readl(S3C_CLK_GATE_PCLK)&~(S3C_CLKCON_PCLK_IIS2), S3C_CLK_GATE_PCLK);
	__raw_writel(__raw_readl(S3C_CLK_GATE_SCLK0)&~(S3C_CLKCON_SCLK0_AUDIO2), S3C_CLK_GATE_SCLK0);

	/* EPLL disable */
	__raw_writel(__raw_readl(S3C_EPLL_CON)&~(1<<31) ,S3C_EPLL_CON);	
...
{
	u32 iismod = readl(s5p64xx_i2s.regs + S3C64XX_IIS0MOD);
	...
	int div_id, int div)
{
	...
	case S3C24XX_DIV_BCLK:
		reg = readl(s5p64xx_i2s.regs + S3C64XX_IIS0MOD) & ~(S3C64XX_IIS0MOD_FS_MASK);
		writel(reg | div, s5p64xx_i2s.regs + S3C64XX_IIS0MOD);
		break;
	case S3C24XX_DIV_PRESCALER:
		if (div)
			div |= 1 << 15;
		writel(div, s5p64xx_i2s.regs + S3C64XX_IIS0PSR);
		break;
	default:
		return -EINVAL;
	}
...
{
	return clk_get_rate(s5p64xx_i2s.iis_clk);
}
	...
	struct snd_soc_dai *dai)
{
	...
	s5p64xx_i2s.regs = ioremap(S5P64XX_PA_IIS_V40, 0x100);
	if (s5p64xx_i2s.regs == NULL)
		return -ENXIO;

	s5p64xx_i2s.iis_clk=clk_get(&pdev->dev, "iis");
	if (s5p64xx_i2s.iis_clk == NULL) {
		printk("failed to get iis_clock\n");
		iounmap(s5p64xx_i2s.regs);
		return -ENODEV;
	}
	clk_enable(s5p64xx_i2s.iis_clk);

	ret = request_irq(IRQ_IISV40, s5p_iis_irq, 0,
			 "s5p-i2s-v40", pdev);
	if (ret < 0) {
...
struct snd_soc_dai s5p_i2s_v40_dai = {
	.name = "s5p-i2s-v40",
	.id = 0,
...
EXPORT_SYMBOL_GPL(s5p_i2s_v40_dai);
