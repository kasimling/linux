...
static int s3c_spi_hw_init(struct s3c_spi *spi)
{
	...
	/* initialize the gpio */
	if (SPI_CHANNEL == 0) {
	        for (gpio = S5PC1XX_GPB(0); gpio < S5PC1XX_GPB(4); gpio++) {
	       		 s3c_gpio_cfgpin(gpio, S3C_GPIO_SFN(2));
		         s3c_gpio_setpull(gpio, S3C_GPIO_PULL_UP);
		} 
	} else if (SPI_CHANNEL == 1) {
                for (gpio = S5PC1XX_GPB(4); gpio <= S5PC1XX_GPB(7); gpio++) {
                         s3c_gpio_cfgpin(gpio, S3C_GPIO_SFN(2));
                         s3c_gpio_setpull(gpio, S3C_GPIO_PULL_UP);
		} 
	} else {
                for (gpio = S5PC1XX_GPG3(0); gpio < S5PC1XX_GPG3(4); gpio++) {
...
static int s3c_spi_dma_init(struct s3c_spi *spi, int mode)
{
	/* Transmission */
	if (mode == 0) 
		s3c2410_dma_devconfig(spi->dma, S3C2410_DMASRC_MEM, 0, S3C_SPI_TX_DATA_REG);
...
static inline void s3c_spi_write_fifo(struct s3c_spi *spi)
{
	...
	if((spi->msg->wbuf) && (spi->msg->flags & SPI_M_MODE_MASTER))
		mdelay(2);
...
static void s3c_spi_message_start(struct s3c_spi *spi)
{
	...
	if (msg->rbuf)
		spi_modecfg |= ( 0x3f << 11); /* Tx FIFO trigger level in INT mode */
	...
	if (msg->flags & SPI_M_DMA_MODE) {
		if (msg->wbuf)
			spi->dma = DMACH_SPI0_OUT;
		if (msg->rbuf)
			spi->dma = DMACH_SPI0_IN;

		if (s3c2410_dma_request(spi->dma, &s3cspi_dma_client, NULL)) {
	...
	switch (spi->state) {
		...
		case STATE_XFER_RX:
			...
			while(!(rx_msgend(spi))){
				...
				if((spi->msg->rbuf) && (spi->msg->flags & SPI_M_MODE_SLAVE))
					mdelay(2);
				DBG("msg_rd_ptr = 0x%x, len = 0x%x \n", spi->msg_rd_ptr ,spi->msg->len);
...
static struct spi_algorithm s3c_spi_algorithm = {
	.name			= "S3C-spi-algorithm",
	.master_xfer		= s3c_spi_xfer,
	.close			= s3c_spi_close,
};

static struct s3c_spi s3c_spi[3] = {
	[0] = {
		.lock	= __SPIN_LOCK_UNLOCKED(s3c_spi[0].lock),
		.spidev	= {
	...
	[1] = {
		.lock	= __SPIN_LOCK_UNLOCKED(s3c_spi[1].lock),
		.spidev	= {
			.algo			= &s3c_spi_algorithm,
			.retries		= 2,
			.timeout		= 5,
        ...
        [2] = {
                .lock   = __SPIN_LOCK_UNLOCKED(s3c_spi[2].lock),
                .spidev = {
                        .algo                   = &s3c_spi_algorithm,
                        .retries                = 2,
                        .timeout                = 5,
...
static int s3c_spi_probe(struct platform_device *pdev)
{
	...
	ret = request_irq(res->start, s3c_spi_irq, IRQF_DISABLED,
			pdev->name, spi);
...
{
	printk(KERN_INFO "S3C SPI Driver \n");
...
MODULE_DESCRIPTION("S3C SPI Bus driver");
