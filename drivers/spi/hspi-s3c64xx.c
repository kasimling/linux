...
static int s3c_spi_hw_init(struct s3c_spi *spi)
{
	...
	if (SPI_CHANNEL == 0) {
		...
		s3c_gpio_cfgpin(S3C64XX_GPC(3), S3C64XX_GPC3_SPI_nCSO);
...
static int s3c_spi_done(struct s3c_spi *spi)
{
	...
	/* initialize the gpio */
	if(SPI_CHANNEL == 0) {
		s3c_gpio_cfgpin(S3C64XX_GPC(0), 0);
		s3c_gpio_cfgpin(S3C64XX_GPC(1), 0);
		s3c_gpio_cfgpin(S3C64XX_GPC(2), 0);
		s3c_gpio_cfgpin(S3C64XX_GPC(3), 0);

	} else {
		s3c_gpio_cfgpin(S3C64XX_GPC(4), 0);
		s3c_gpio_cfgpin(S3C64XX_GPC(5), 0);
		s3c_gpio_cfgpin(S3C64XX_GPC(6), 0);
		s3c_gpio_cfgpin(S3C64XX_GPC(7), 0);
	}
