		...
		)
{
	...
	switch (args->get_config.in_config_param) {
		...
		case MFC_GET_CONFIG_DEC_BYTE_CONSUMED:
			...
			printk(KERN_DEBUG "\n%s: MFC_GET_CONFIG_DEC_BYTE_CONSUMED = %d\n", __FUNCTION__, (int)pMfcInst->RET_DEC_PIC_RUN_BAK_BYTE_CONSUMED);
			ret = MFCINST_RET_OK;
			break;
		...
		)
{
	...
	switch (args->set_config.in_config_param) {
...
#else
			printk(KERN_ERR "\n%s: IOCTL_MFC_SET_CONFIG with MFC_SET_CONFIG_DEC_ROTATE is not supported\n", __FUNCTION__);
			printk(KERN_ERR "\n%s: please check if MFC_ROTATE_ENABLE is defined as 1 in MfcConfig.h file\n", __FUNCTION__);
#endif
			ret = MFCINST_RET_OK;
			break;
		...
		case MFC_SET_CONFIG_ENC_H263_PARAM:
			...
			printk(KERN_DEBUG "\n%s: parameter = 0x%x\n", __FUNCTION__, pMfcInst->h263_annex);		
			ret = MFCINST_RET_OK;
			break;
