		...
		int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case IOCTL_MFC_H263_ENC_INIT:
			...
			out = copy_from_user(&args.enc_init, (MFC_ENC_INIT_ARG *)arg, sizeof(MFC_ENC_INIT_ARG));
			...
			out = copy_to_user((MFC_ENC_INIT_ARG *)arg, &args.enc_init, sizeof(MFC_ENC_INIT_ARG));
		...
		case IOCTL_MFC_H263_ENC_EXE:
			...
			out = copy_from_user(&args.enc_exe, (MFC_ENC_EXE_ARG *)arg, sizeof(MFC_ENC_EXE_ARG));
			...
			out = copy_to_user((MFC_ENC_EXE_ARG *)arg, &args.enc_exe, sizeof(MFC_ENC_EXE_ARG));
		...
		case IOCTL_MFC_VC1_DEC_INIT:
			...
			out = copy_from_user(&args.dec_init, (MFC_DEC_INIT_ARG *)arg, sizeof(MFC_DEC_INIT_ARG));
			...
			out = copy_to_user((MFC_DEC_INIT_ARG *)arg, &args.dec_init, sizeof(MFC_DEC_INIT_ARG));
		...
		case IOCTL_MFC_VC1_DEC_EXE:
			...
			out = copy_from_user(&args.dec_exe, (MFC_DEC_EXE_ARG *)arg, sizeof(MFC_DEC_EXE_ARG));
			...
			out = copy_to_user((MFC_DEC_EXE_ARG *)arg, &args.dec_exe, sizeof(MFC_DEC_EXE_ARG));
		...
		case IOCTL_MFC_GET_LINE_BUF_ADDR:
			...
			out = copy_from_user(&args.get_buf_addr, (MFC_GET_BUF_ADDR_ARG *)arg, sizeof(MFC_GET_BUF_ADDR_ARG));
			...
			out = copy_to_user((MFC_GET_BUF_ADDR_ARG *)arg, &args.get_buf_addr, sizeof(MFC_GET_BUF_ADDR_ARG));
...
//			args.get_buf_addr.out_buf_size	= ((pMfcInst->width+2*DIVX_PADDING) * (pMfcInst->height+2*DIVX_PADDING) * 3) >> 1;
			tmp	= (unsigned int)GetDataBufPhyAddr() + ( ((unsigned int) pMfcInst->pFramBuf)	\
				+ (pMfcInst->run_index) * (args.get_buf_addr.out_buf_size) - (unsigned int)GetDataBufVirAddr() );
...
//.] sichoi 081103
		...
		case IOCTL_MFC_GET_MPEG4_ASP_PARAM:
			...
			out = copy_from_user(&args.mpeg4_asp_param, (MFC_GET_MPEG4ASP_ARG *)arg, sizeof(MFC_GET_MPEG4ASP_ARG));
			...
			out = copy_to_user((MFC_GET_MPEG4ASP_ARG *)arg, &args.mpeg4_asp_param, sizeof(MFC_GET_MPEG4ASP_ARG));
		...
		case IOCTL_MFC_GET_CONFIG:
			...
			out = copy_from_user(&args, (MFC_ARGS *)arg, sizeof(MFC_ARGS));

			ret = MFC_GetConfigParams(pMfcInst, &args);

			out = copy_to_user((MFC_ARGS *)arg, &args, sizeof(MFC_ARGS));
...
int s3c_mfc_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	pageFrameNo = __phys_to_pfn(s3c_mfc_phys_data_buffer + MFC_BITPROC_BUF_SIZE);
...
static int s3c_mfc_probe(struct platform_device *pdev)
{
	...
	//mfc_base = ioremap_nocache(res->start, size);
	...
	/* memory secure */
	s3c_mfc_phys_data_buffer = s3c_get_media_memory(S3C_MDEV_MFC);
	s3c_mfc_virt_data_buffer = ioremap_nocache(s3c_mfc_phys_data_buffer, s3c_get_media_memsize(S3C_MDEV_MFC));
...
//			mfc_save[index] = (unsigned int)MFC_READ_REG(dwMfcBase + i);
			mfc_save[index] = readl(dwMfcBase + i);
			index++;	
		}
...
//			MFC_WRITE_REG( ( dwMfcBase + i ), mfc_save[index] );
			writel(mfc_save[index], dwMfcBase + i);
			index++;
		}
