...
BOOL FramBufMgrInit(unsigned char *pBufBase, int nBufSize)
{
	...
	_p_segment_info = (SEGMENT_INFO *) kmalloc(_nNumSegs * sizeof(SEGMENT_INFO), GFP_KERNEL);
	for (i=0; i<_nNumSegs; i++) {
		_p_segment_info[i].pBaseAddr   = pBufBase  +  (i * BUF_SEGMENT_SIZE);
		_p_segment_info[i].idx_commit  = 0;
	}

	_p_commit_info  = (COMMIT_INFO *) kmalloc(_nNumSegs * sizeof(COMMIT_INFO), GFP_KERNEL);
	for (i=0; i<_nNumSegs; i++) {
...
void FramBufMgrPrintCommitInfo()
{
	...
	for (i=0; i<_nNumSegs; i++) {
		if (_p_commit_info[i].index_base_seg != -1)  {
			printk(KERN_DEBUG, "\n%s: commit index = %03d, base segment index = %d\n", __FUNCTION__, i, _p_commit_info[i].index_base_seg);
			printk(KERN_DEBUG, "\n%s: commit index = %03d, number of segment = %d\n", __FUNCTION__, i, _p_commit_info[i].num_segs);			
		}
	}
}
