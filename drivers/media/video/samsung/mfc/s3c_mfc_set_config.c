...
int s3c_mfc_get_config_params(
		s3c_mfc_instance_context_t  *mfc_inst,
		s3c_mfc_args_t     *args )		/* Input arguments for S3C_MFC_IOCTL_MFC_SET_CONFIG */
{
	int             ret;
		...
		s3c_mfc_args_t     *args)		/* Input arguments for S3C_MFC_IOCTL_MFC_SET_CONFIG */
{
	...
	switch (args->set_config.in_config_param) {
		...
		case S3C_MFC_SET_CACHE_CLEAN:
			 ...
			 */
			start = (unsigned int)mfc_inst->stream_buffer + (args->set_config.in_config_value[0] - args->set_config.in_config_value[2]);
